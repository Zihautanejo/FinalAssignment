    ...
    public class ComEvaService
    {
        ...
        public List<ComEvaluation> QueryComEvaByStudent(string studentid, string semester)
        {
            return DbContext.ComEvaluations
                ...
                .Where(c=>c.Semester == (semester==null?"*":semester))
                .ToList();
        }
        ...
        //个人成绩降序
        public List<ComEvaluation> QueryComEvaByMajorOrderGradeDown(List<ComEvaluation> list)
        {
            return DbContext.ComEvaluations
                .Include(c => c.A)
                .Include(c => c.B1)
        ...
        //根据classId返回综测列表
        public List<ComEvaluation> QueryComEvaByClassId(string classId)
        {
            return DbContext.ComEvaluations
                .Include(c => c.A)
                .Include(c => c.B1)
        ...
        //修改每个班级的未交人数和平均分
        public void ModifyClassIfo(List<string> c)
        {
            foreach (string cid in c)
            {
                List<ComEvaluation> com = QueryComEvaByClassId(cid);
                int num = QueryUnSubmit(com);
                double avg = QueryAVEGrade(com);
                classService.ModifyAvaPoint(cid, avg);
                classService.ModifyNotSubNum(cid, num);
            }
        }
